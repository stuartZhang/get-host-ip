...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    #[cfg(debug_assertions)]
    dbg!(&ip_config_pojo);
    if let Some(value) = ip_config_pojo
        .get(&cli_args.section[..])
        .and_then(|section| section.get(&cli_args.entry[..]))
    {
        print!("{value}")
    }
    Ok(())
    ...
    Ok(input_str
        ...
        .fold(HashMap::new(), |mut map, line| {
            if section.is_match(line) {
                ...
                last_section_key
                    .as_ref()
                    .and_then(|section_key| map.get_mut(section_key))
                    .map(|sub_map| {
